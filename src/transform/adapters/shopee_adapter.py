"""
transform.adapters.shopee_adapter
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
class ShopeeAdapter(BaseMarketplaceAdapter):
    ...
    origem_venda: str = "SHOPEE"
    transacao_prefix: str = "SHP"
    ...
    @staticmethod
    def _unix_to_date_str(unix_ts: int) -> str:
        if not unix_ts:
            return ""
    ...
    def padronizar_clientes(self) -> List[Dict[str, Any]]:
        ...
        for pedido in self.raw_data:
            ...
            clientes.append({
                ...
                "nickname": truncar(
                    pedido.get("buyer_username") or "CLIENTE NÃO INFORMADO", 100
                ),
                "nome_completo": "",
            })
        return clientes
    ...
    def padronizar_pedidos(self) -> List[Dict[str, Any]]:
        ...
        for pedido in self.raw_data:
            ...
            escrow = pedido.get("escrow_detail", {}) or {}
            order_income = escrow.get("order_income", {}) or {}
            buyer_info = escrow.get("buyer_payment_info", {}) or {}

            total_pago = float(
                buyer_info.get("buyer_total_amount")
                or order_income.get("buyer_total_amount")
                or pedido.get("total_amount", 0.0)
                or 0.0
            )

            data_criacao = self._unix_to_str(pedido.get("create_time", 0))

            item_list = pedido.get("item_list", []) or []
            valor_produtos = sum(
                int(item.get("model_quantity_purchased") or 1)
                * float(item.get("model_discounted_price") or 0.0)
    ...
    def padronizar_itens(self) -> List[Dict[str, Any]]:
        ...
        for pedido in self.raw_data:
            order_sn = str(pedido.get("order_sn", ""))
            for item in pedido.get("item_list", []) or []:
                itens.append({
                    "id_pedido": order_sn,
    ...
    def padronizar_transacoes(self) -> List[Dict[str, Any]]:
        ...
        for pedido in self.raw_data:
            ...
            for categoria, valor in taxas.items():
                if valor != 0.0:
                    transacoes.append({
                        "id_transacao": f"{self.transacao_prefix}-{order_sn}-{categoria}",
                        "id_pedido": order_sn,
                        "id_canal": self.id_canal,
        ...
        for pedido in self.raw_data:
            for item in pedido.get("item_list", []) or []:
                anuncios.append({
                    "id_anuncio": str(item.get("item_id", "")),
                    "id_canal": self.id_canal,
                    "sku": truncar(item.get("item_sku", ""), 100),
                    "titulo_anuncio": truncar(item.get("item_name", ""), 255),
                    "tipo_anuncio": "",
                })
        return anuncios
